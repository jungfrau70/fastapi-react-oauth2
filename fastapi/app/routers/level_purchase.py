...
@router.post("/create")
async def create(request: pydantic_level_purchase_in,
                 user: pydantic_user = Depends(get_current_user)):
    obj = ""
    try:
        obj = await LevelPurchase.filter(owner=user.id).first()
        profile_obj = await UserProfile.filter(owner=user.id).first()
    except:
        pass
    ...
    if True:
        ...
        if request:
            ...
            request["purchase_date"] = round(datetime.timestamp(t_now))
            request["expiration_date"] = round(
                datetime.timestamp(expiration_date))
            obj = await LevelPurchase.create(**request, owner=user)

            profile_obj.play_ticket = profile_obj.play_ticket - 1
...
@router.get('/all')
async def read_all():
    response = await pydantic_level_purchase_out.from_queryset(LevelPurchase.all())
    if response:
...
async def read_one(uuid: str,
    ...
    if owner == logined_user.id:
        response = await pydantic_level_purchase_out.from_queryset_single(LevelPurchase.get(id=uuid))
        if response:
            return {"status": "ok", "data": response}
        else:
            return {"status": "error"}
...
async def update(uuid: str,
                 request: pydantic_level_purchase_in,
                 user: pydantic_user = Depends(get_current_user)):

    try:
        obj = await LevelPurchase.filter(id=uuid).first()
        owner = obj.owner_id
...
async def delete(uuid: str,
    ...
    if owner == logined_user.id:
        obj = await LevelPurchase.filter(id=uuid).first().delete()
        if obj:
            return {
                "status": "ok",
                "message": "deleted"
...
async def read_mine(user: pydantic_user_in = Depends(get_current_user)):
    # print(user)
    response = await LevelPurchase.filter(owner_id=user.id)
