...
@router.post("/create")
async def create(request: pydantic_product_purchase_in,
    ...
    if True:
        ...
        if request:
            ...
            obj = await ProductPurchase.create(**request, owner=user)
            profile_obj.play_ticket = profile_obj.play_ticket + 100
            # print(profile_obj.play_ticket)
...
@router.get('/all')
async def read_all():
    response = await pydantic_product_purchase_in.from_queryset(ProductPurchase.all())
    if response:
...
async def read_one(uuid: str,
                   user: pydantic_user = Depends(get_current_user)):
    obj = await ProductPurchase.filter(owner=user.id).first()
    owner = obj.owner_id

    logined_user = await User.get(id=user.id)

    if owner == logined_user.id:
        response = await pydantic_product_purchase_in.from_queryset_single(ProductPurchase.get(id=uuid))
        if response:
            return {"status": "ok", "data": response}
        else:
            return {"status": "error"}
...
async def update(uuid: str,
                 request: pydantic_product_purchase_in,
                 user: pydantic_user = Depends(get_current_user)):
    try:
        obj = await ProductPurchase.filter(id=uuid).first()
        owner = obj.owner_id
...
async def delete(uuid: str,
    ...
    if owner == logined_user.id:
        obj = await ProductPurchase.filter(id=uuid).first().delete()
        if obj:
            return {
                "status": "ok",
                "message": "deleted"
...
async def read_mine(user: pydantic_user_in = Depends(get_current_user)):
    response = await ProductPurchase.filter(owner_id=user.id)
