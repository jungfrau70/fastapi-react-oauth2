...
@router.post("/create")
async def create(request: pydantic_ar_view_in,
                 user: pydantic_user = Depends(get_current_user)):
    obj = ""
    try:
        obj = await ARView.filter(owner=user.id).first()
    except:
        pass
    ...
    if True:
        ...
        if request:
            obj = await ARView.create(**request, owner=user)
...
@router.get('/all')
async def read_all():
    response = await pydantic_ar_view_out.from_queryset(ARView.all())
    if response:
...
async def read_one(uuid: str,
    ...
    if owner == logined_user.id:
        response = await pydantic_ar_view_out.from_queryset_single(ARView.get(id=uuid))
        if response:
            return {"status": "ok", "data": response}
        else:
            return {"status": "error"}
...
async def update(uuid: str,
                 request: pydantic_ar_view_in,
                 user: pydantic_user = Depends(get_current_user)):

    try:
        obj = await ARView.filter(id=uuid).first()
        owner = obj.owner_id
...
async def delete(uuid: str,
    ...
    if owner == logined_user.id:
        obj = await ARView.filter(id=uuid).first().delete()
        if obj:
            return {
                "status": "ok",
                "message": "deleted"
...
async def read_mine(user: pydantic_user_in = Depends(get_current_user)):
    response = await ARView.filter(owner_id=user.id)
